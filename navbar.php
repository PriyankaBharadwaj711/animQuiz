      ...
      <div class="collapse" style="position: fixed;padding: 10px;z-index: 99999999;right: 0px;color: white;background: rgba(76, 84, 138, 0.23);top: 50px;" id="collapseLogin" >
       <form name="myForm" role="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post"  onsubmit="return validateLogin();">
          ...
          <div style="text-align:center">
            <button type="submit" id="loginBtn" class="btn btn-success"><span class="glyphicon glyphicon-log-in"></span></button>
            <div><a href="./registration.php?btn=back" style="color:blue; float:right; padding:8px;"></a></div>
          <div><a href="./forgotPassword.php?btn=back" style="color:orange; float:right; padding:8px;"></a></div>
...
<div class="topnav" id="myTopnav">
    <div>
      ...
      if ($_SESSION["rname"]!="Student"){
          ?>
        <a href="./sdoh.php?version=".$_SESSION['version'] class="active">Lifescreen Animated Tool</a>
        <?php
        }else {
          ?>
        ...
        if ($_SESSION["f1"]) {
        ?>
        <a href="./accessCtrl.php"></a>
        <?php
        }
        if ($_SESSION["f2"]) {
        ?>
        <a href="./quizSubmissions.php"></a>
        <?php
       }
        if ($_SESSION["f3"]) {
        ?>
        <a href="./userActivity.php"></a>
        <?php
      }
      if ($_SESSION["rname"]!="Student"){
        ?>
